...
bool InitPlayer(int& iJobSelector, Player* player, FILE** fp);
...
void SavePlayer(FILE** fp, Player* player);
void LoadPlayer(FILE** fp, Player* player);
	...
	while (!isGameOver)
	{
		...
		case JobSelect:
			...
			while (!isSelected)
			{
				DrawJobSelect();
				cin >> iJobSelector;
				isSelected = InitPlayer(iJobSelector, &player, &fp);
				std::system("cls");
			}
...
void DrawDifficulty()
{
	...
	cout << "1.쉬움\t2.보통\t3.어려움\t4.종료" << endl;
	cout << endl;
	cout << "**********************************" << endl;
}
